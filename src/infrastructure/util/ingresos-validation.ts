...
type ValidationIngresoData = {
  error: Joi.ValidationError | undefined;
  value: Ingreso;
};

function validateIngresoData(data: any): ValidationIngresoData {
  ...
  const { error, value } = ingresoSchema.validate(data, { abortEarly: false });
  return { error, value };
}

export const loadIngresoData = (data: any): Ingreso => {
  const result = validateIngresoData(data);
  if (result.error) {
