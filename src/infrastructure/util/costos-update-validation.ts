...
const joi = Joi;
export type ReturnUpdateCostoData = Partial<{
  id: number;
  description: string;
  value: number;
  date: Date;
...
type ValidationUpdateCostoData = {
  error: Joi.ValidationError | undefined;
  value: ReturnUpdateCostoData;
};
function validateUpdateCostoData(data: any): ValidationUpdateCostoData {
  const CostoSchema = Joi.object({
    ...
    .unknown(false)
    .or('id', 'description', 'value', 'date', 'source', 'empId', 'usuId'); //no se permiten campos extra u otros fields no conocidos
  //el or se agregó para que no se limite a exigir todo sino que permita que no sean todos los valores los que se modifiquen
  const { error, value } = CostoSchema.validate(data, {
    abortEarly: false,
    stripUnknown: true,
    convert: true,
...
export const loadUpdateCostoData = (data: any): ReturnUpdateCostoData => {
  const result = validateUpdateCostoData(data);
  if (result.error) {
